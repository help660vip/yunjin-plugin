...
const service = serviceFor('38');
const contract = contractFor('38');

test('38 normal command contract', () => {
  assert.equal(manifest.id, '38');
  assert.equal(service.featureId, '38');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('38 invalid argument boundary', () => {
...
test('38 permission contract is explicit', () => {
...
test('38 scope and storage contract are bounded', () => {
...
test('38 dependency and renderer fallback are declared', () => {
...
test('38 redacts long metadata without changing command identity', () => {
...
test('38 failure matrix contains required acceptance paths', () => {
