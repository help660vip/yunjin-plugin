...
const service = serviceFor('17');
const contract = contractFor('17');

test('17 normal command contract', () => {
  assert.equal(manifest.id, '17');
  assert.equal(service.featureId, '17');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('17 invalid argument boundary', () => {
...
test('17 permission contract is explicit', () => {
...
test('17 scope and storage contract are bounded', () => {
...
test('17 dependency and renderer fallback are declared', () => {
...
test('17 redacts long metadata without changing command identity', () => {
...
test('17 failure matrix contains required acceptance paths', () => {
