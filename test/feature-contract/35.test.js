...
const service = serviceFor('35');
const contract = contractFor('35');

test('35 normal command contract', () => {
  assert.equal(manifest.id, '35');
  assert.equal(service.featureId, '35');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('35 invalid argument boundary', () => {
...
test('35 permission contract is explicit', () => {
...
test('35 scope and storage contract are bounded', () => {
...
test('35 dependency and renderer fallback are declared', () => {
...
test('35 redacts long metadata without changing command identity', () => {
...
test('35 failure matrix contains required acceptance paths', () => {
