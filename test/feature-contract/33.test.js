...
const service = serviceFor('33');
const contract = contractFor('33');

test('33 normal command contract', () => {
  assert.equal(manifest.id, '33');
  assert.equal(service.featureId, '33');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('33 invalid argument boundary', () => {
...
test('33 permission contract is explicit', () => {
...
test('33 scope and storage contract are bounded', () => {
...
test('33 dependency and renderer fallback are declared', () => {
...
test('33 redacts long metadata without changing command identity', () => {
...
test('33 failure matrix contains required acceptance paths', () => {
