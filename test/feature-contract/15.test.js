...
const service = serviceFor('15');
const contract = contractFor('15');

test('15 normal command contract', () => {
  assert.equal(manifest.id, '15');
  assert.equal(service.featureId, '15');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('15 invalid argument boundary', () => {
...
test('15 permission contract is explicit', () => {
...
test('15 scope and storage contract are bounded', () => {
...
test('15 dependency and renderer fallback are declared', () => {
...
test('15 redacts long metadata without changing command identity', () => {
...
test('15 failure matrix contains required acceptance paths', () => {
