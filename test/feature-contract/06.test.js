...
const service = serviceFor('06');
const contract = contractFor('06');

test('06 normal command contract', () => {
  assert.equal(manifest.id, '06');
  assert.equal(service.featureId, '06');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('06 invalid argument boundary', () => {
...
test('06 permission contract is explicit', () => {
...
test('06 scope and storage contract are bounded', () => {
...
test('06 dependency and renderer fallback are declared', () => {
...
test('06 redacts long metadata without changing command identity', () => {
...
test('06 failure matrix contains required acceptance paths', () => {
