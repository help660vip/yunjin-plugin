...
const service = serviceFor('22');
const contract = contractFor('22');

test('22 normal command contract', () => {
  assert.equal(manifest.id, '22');
  assert.equal(service.featureId, '22');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('22 invalid argument boundary', () => {
...
test('22 permission contract is explicit', () => {
...
test('22 scope and storage contract are bounded', () => {
...
test('22 dependency and renderer fallback are declared', () => {
...
test('22 redacts long metadata without changing command identity', () => {
...
test('22 failure matrix contains required acceptance paths', () => {
