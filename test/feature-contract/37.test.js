...
const service = serviceFor('37');
const contract = contractFor('37');

test('37 normal command contract', () => {
  assert.equal(manifest.id, '37');
  assert.equal(service.featureId, '37');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('37 invalid argument boundary', () => {
...
test('37 permission contract is explicit', () => {
...
test('37 scope and storage contract are bounded', () => {
...
test('37 dependency and renderer fallback are declared', () => {
...
test('37 redacts long metadata without changing command identity', () => {
...
test('37 failure matrix contains required acceptance paths', () => {
