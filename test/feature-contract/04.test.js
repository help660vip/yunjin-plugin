...
const service = serviceFor('04');
const contract = contractFor('04');

test('04 normal command contract', () => {
  assert.equal(manifest.id, '04');
  assert.equal(service.featureId, '04');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('04 invalid argument boundary', () => {
...
test('04 permission contract is explicit', () => {
...
test('04 scope and storage contract are bounded', () => {
...
test('04 dependency and renderer fallback are declared', () => {
...
test('04 redacts long metadata without changing command identity', () => {
...
test('04 failure matrix contains required acceptance paths', () => {
