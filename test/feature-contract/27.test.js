...
const service = serviceFor('27');
const contract = contractFor('27');

test('27 normal command contract', () => {
  assert.equal(manifest.id, '27');
  assert.equal(service.featureId, '27');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('27 invalid argument boundary', () => {
...
test('27 permission contract is explicit', () => {
...
test('27 scope and storage contract are bounded', () => {
...
test('27 dependency and renderer fallback are declared', () => {
...
test('27 redacts long metadata without changing command identity', () => {
...
test('27 failure matrix contains required acceptance paths', () => {
