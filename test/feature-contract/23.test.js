...
const service = serviceFor('23');
const contract = contractFor('23');

test('23 normal command contract', () => {
  assert.equal(manifest.id, '23');
  assert.equal(service.featureId, '23');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('23 invalid argument boundary', () => {
...
test('23 permission contract is explicit', () => {
...
test('23 scope and storage contract are bounded', () => {
...
test('23 dependency and renderer fallback are declared', () => {
...
test('23 redacts long metadata without changing command identity', () => {
...
test('23 failure matrix contains required acceptance paths', () => {
