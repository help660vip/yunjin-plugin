...
const service = serviceFor('14');
const contract = contractFor('14');

test('14 normal command contract', () => {
  assert.equal(manifest.id, '14');
  assert.equal(service.featureId, '14');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('14 invalid argument boundary', () => {
...
test('14 permission contract is explicit', () => {
...
test('14 scope and storage contract are bounded', () => {
...
test('14 dependency and renderer fallback are declared', () => {
...
test('14 redacts long metadata without changing command identity', () => {
...
test('14 failure matrix contains required acceptance paths', () => {
