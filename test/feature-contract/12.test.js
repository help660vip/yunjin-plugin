...
const service = serviceFor('12');
const contract = contractFor('12');

test('12 normal command contract', () => {
  assert.equal(manifest.id, '12');
  assert.equal(service.featureId, '12');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('12 invalid argument boundary', () => {
...
test('12 permission contract is explicit', () => {
...
test('12 scope and storage contract are bounded', () => {
...
test('12 dependency and renderer fallback are declared', () => {
...
test('12 redacts long metadata without changing command identity', () => {
...
test('12 failure matrix contains required acceptance paths', () => {
