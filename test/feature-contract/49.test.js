...
const service = serviceFor('49');
const contract = contractFor('49');

test('49 normal command contract', () => {
  assert.equal(manifest.id, '49');
  assert.equal(service.featureId, '49');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('49 invalid argument boundary', () => {
...
test('49 permission contract is explicit', () => {
...
test('49 scope and storage contract are bounded', () => {
...
test('49 dependency and renderer fallback are declared', () => {
...
test('49 redacts long metadata without changing command identity', () => {
...
test('49 failure matrix contains required acceptance paths', () => {
