...
const service = serviceFor('21');
const contract = contractFor('21');

test('21 normal command contract', () => {
  assert.equal(manifest.id, '21');
  assert.equal(service.featureId, '21');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('21 invalid argument boundary', () => {
...
test('21 permission contract is explicit', () => {
...
test('21 scope and storage contract are bounded', () => {
...
test('21 dependency and renderer fallback are declared', () => {
...
test('21 redacts long metadata without changing command identity', () => {
...
test('21 failure matrix contains required acceptance paths', () => {
