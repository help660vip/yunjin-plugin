...
const service = serviceFor('40');
const contract = contractFor('40');

test('40 normal command contract', () => {
  assert.equal(manifest.id, '40');
  assert.equal(service.featureId, '40');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('40 invalid argument boundary', () => {
...
test('40 permission contract is explicit', () => {
...
test('40 scope and storage contract are bounded', () => {
...
test('40 dependency and renderer fallback are declared', () => {
...
test('40 redacts long metadata without changing command identity', () => {
...
test('40 failure matrix contains required acceptance paths', () => {
