...
const service = serviceFor('26');
const contract = contractFor('26');

test('26 normal command contract', () => {
  assert.equal(manifest.id, '26');
  assert.equal(service.featureId, '26');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('26 invalid argument boundary', () => {
...
test('26 permission contract is explicit', () => {
...
test('26 scope and storage contract are bounded', () => {
...
test('26 dependency and renderer fallback are declared', () => {
...
test('26 redacts long metadata without changing command identity', () => {
...
test('26 failure matrix contains required acceptance paths', () => {
