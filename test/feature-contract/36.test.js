...
const service = serviceFor('36');
const contract = contractFor('36');

test('36 normal command contract', () => {
  assert.equal(manifest.id, '36');
  assert.equal(service.featureId, '36');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('36 invalid argument boundary', () => {
...
test('36 permission contract is explicit', () => {
...
test('36 scope and storage contract are bounded', () => {
...
test('36 dependency and renderer fallback are declared', () => {
...
test('36 redacts long metadata without changing command identity', () => {
...
test('36 failure matrix contains required acceptance paths', () => {
