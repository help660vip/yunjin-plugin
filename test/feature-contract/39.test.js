...
const service = serviceFor('39');
const contract = contractFor('39');

test('39 normal command contract', () => {
  assert.equal(manifest.id, '39');
  assert.equal(service.featureId, '39');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('39 invalid argument boundary', () => {
...
test('39 permission contract is explicit', () => {
...
test('39 scope and storage contract are bounded', () => {
...
test('39 dependency and renderer fallback are declared', () => {
...
test('39 redacts long metadata without changing command identity', () => {
...
test('39 failure matrix contains required acceptance paths', () => {
