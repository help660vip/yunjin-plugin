...
const service = serviceFor('30');
const contract = contractFor('30');

test('30 normal command contract', () => {
  assert.equal(manifest.id, '30');
  assert.equal(service.featureId, '30');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('30 invalid argument boundary', () => {
...
test('30 permission contract is explicit', () => {
...
test('30 scope and storage contract are bounded', () => {
...
test('30 dependency and renderer fallback are declared', () => {
...
test('30 redacts long metadata without changing command identity', () => {
...
test('30 failure matrix contains required acceptance paths', () => {
