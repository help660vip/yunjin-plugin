...
const service = serviceFor('48');
const contract = contractFor('48');

test('48 normal command contract', () => {
  assert.equal(manifest.id, '48');
  assert.equal(service.featureId, '48');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('48 invalid argument boundary', () => {
...
test('48 permission contract is explicit', () => {
...
test('48 scope and storage contract are bounded', () => {
...
test('48 dependency and renderer fallback are declared', () => {
...
test('48 redacts long metadata without changing command identity', () => {
...
test('48 failure matrix contains required acceptance paths', () => {
