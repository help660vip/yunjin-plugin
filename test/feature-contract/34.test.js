...
const service = serviceFor('34');
const contract = contractFor('34');

test('34 normal command contract', () => {
  assert.equal(manifest.id, '34');
  assert.equal(service.featureId, '34');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('34 invalid argument boundary', () => {
...
test('34 permission contract is explicit', () => {
...
test('34 scope and storage contract are bounded', () => {
...
test('34 dependency and renderer fallback are declared', () => {
...
test('34 redacts long metadata without changing command identity', () => {
...
test('34 failure matrix contains required acceptance paths', () => {
