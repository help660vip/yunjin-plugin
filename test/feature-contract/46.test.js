...
const service = serviceFor('46');
const contract = contractFor('46');

test('46 normal command contract', () => {
  assert.equal(manifest.id, '46');
  assert.equal(service.featureId, '46');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('46 invalid argument boundary', () => {
...
test('46 permission contract is explicit', () => {
...
test('46 scope and storage contract are bounded', () => {
...
test('46 dependency and renderer fallback are declared', () => {
...
test('46 redacts long metadata without changing command identity', () => {
...
test('46 failure matrix contains required acceptance paths', () => {
