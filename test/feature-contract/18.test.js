...
const service = serviceFor('18');
const contract = contractFor('18');

test('18 normal command contract', () => {
  assert.equal(manifest.id, '18');
  assert.equal(service.featureId, '18');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('18 invalid argument boundary', () => {
...
test('18 permission contract is explicit', () => {
...
test('18 scope and storage contract are bounded', () => {
...
test('18 dependency and renderer fallback are declared', () => {
...
test('18 redacts long metadata without changing command identity', () => {
...
test('18 failure matrix contains required acceptance paths', () => {
