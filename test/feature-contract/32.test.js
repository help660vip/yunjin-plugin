...
const service = serviceFor('32');
const contract = contractFor('32');

test('32 normal command contract', () => {
  assert.equal(manifest.id, '32');
  assert.equal(service.featureId, '32');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('32 invalid argument boundary', () => {
...
test('32 permission contract is explicit', () => {
...
test('32 scope and storage contract are bounded', () => {
...
test('32 dependency and renderer fallback are declared', () => {
...
test('32 redacts long metadata without changing command identity', () => {
...
test('32 failure matrix contains required acceptance paths', () => {
