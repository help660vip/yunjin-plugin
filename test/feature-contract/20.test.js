...
const service = serviceFor('20');
const contract = contractFor('20');

test('20 normal command contract', () => {
  assert.equal(manifest.id, '20');
  assert.equal(service.featureId, '20');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('20 invalid argument boundary', () => {
...
test('20 permission contract is explicit', () => {
...
test('20 scope and storage contract are bounded', () => {
...
test('20 dependency and renderer fallback are declared', () => {
...
test('20 redacts long metadata without changing command identity', () => {
...
test('20 failure matrix contains required acceptance paths', () => {
