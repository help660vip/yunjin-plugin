...
const service = serviceFor('42');
const contract = contractFor('42');

test('42 normal command contract', () => {
  assert.equal(manifest.id, '42');
  assert.equal(service.featureId, '42');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('42 invalid argument boundary', () => {
...
test('42 permission contract is explicit', () => {
...
test('42 scope and storage contract are bounded', () => {
...
test('42 dependency and renderer fallback are declared', () => {
...
test('42 redacts long metadata without changing command identity', () => {
...
test('42 failure matrix contains required acceptance paths', () => {
