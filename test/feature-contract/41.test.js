...
const contract = contractFor('41');

test('41 normal command contract', () => {
  assert.equal(manifest.id, '41');
  assert.equal(service.featureId, '41');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('41 invalid argument boundary', () => {
...
test('41 permission contract is explicit', () => {
...
test('41 scope and storage contract are bounded', () => {
...
test('41 dependency and renderer fallback are declared', () => {
...
test('41 redacts long metadata without changing command identity', () => {
...
test('41 failure matrix contains required acceptance paths', () => {
