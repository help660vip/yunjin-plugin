...
const service = serviceFor('44');
const contract = contractFor('44');

test('44 normal command contract', () => {
  assert.equal(manifest.id, '44');
  assert.equal(service.featureId, '44');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('44 invalid argument boundary', () => {
...
test('44 permission contract is explicit', () => {
...
test('44 scope and storage contract are bounded', () => {
...
test('44 dependency and renderer fallback are declared', () => {
...
test('44 redacts long metadata without changing command identity', () => {
...
test('44 failure matrix contains required acceptance paths', () => {
