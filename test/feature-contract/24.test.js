...
const service = serviceFor('24');
const contract = contractFor('24');

test('24 normal command contract', () => {
  assert.equal(manifest.id, '24');
  assert.equal(service.featureId, '24');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('24 invalid argument boundary', () => {
...
test('24 permission contract is explicit', () => {
...
test('24 scope and storage contract are bounded', () => {
...
test('24 dependency and renderer fallback are declared', () => {
...
test('24 redacts long metadata without changing command identity', () => {
...
test('24 failure matrix contains required acceptance paths', () => {
