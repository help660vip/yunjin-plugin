...
const service = serviceFor('08');
const contract = contractFor('08');

test('08 normal command contract', () => {
  assert.equal(manifest.id, '08');
  assert.equal(service.featureId, '08');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('08 invalid argument boundary', () => {
...
test('08 permission contract is explicit', () => {
...
test('08 scope and storage contract are bounded', () => {
...
test('08 dependency and renderer fallback are declared', () => {
...
test('08 redacts long metadata without changing command identity', () => {
...
test('08 failure matrix contains required acceptance paths', () => {
