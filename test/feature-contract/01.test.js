...
const contract = contractFor('01');

test('01 normal command contract', () => {
  assert.equal(manifest.id, '01');
  assert.equal(service.featureId, '01');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('01 invalid argument boundary', () => {
...
test('01 permission contract is explicit', () => {
...
test('01 scope and storage contract are bounded', () => {
...
test('01 dependency and renderer fallback are declared', () => {
...
test('01 redacts long metadata without changing command identity', () => {
...
test('01 failure matrix contains required acceptance paths', () => {
