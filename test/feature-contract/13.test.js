...
const service = serviceFor('13');
const contract = contractFor('13');

test('13 normal command contract', () => {
  assert.equal(manifest.id, '13');
  assert.equal(service.featureId, '13');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('13 invalid argument boundary', () => {
...
test('13 permission contract is explicit', () => {
...
test('13 scope and storage contract are bounded', () => {
...
test('13 dependency and renderer fallback are declared', () => {
...
test('13 redacts long metadata without changing command identity', () => {
...
test('13 failure matrix contains required acceptance paths', () => {
