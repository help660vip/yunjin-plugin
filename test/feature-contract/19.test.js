...
const service = serviceFor('19');
const contract = contractFor('19');

test('19 normal command contract', () => {
  assert.equal(manifest.id, '19');
  assert.equal(service.featureId, '19');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('19 invalid argument boundary', () => {
...
test('19 permission contract is explicit', () => {
...
test('19 scope and storage contract are bounded', () => {
...
test('19 dependency and renderer fallback are declared', () => {
...
test('19 redacts long metadata without changing command identity', () => {
...
test('19 failure matrix contains required acceptance paths', () => {
