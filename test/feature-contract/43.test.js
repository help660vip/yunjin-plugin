...
const service = serviceFor('43');
const contract = contractFor('43');

test('43 normal command contract', () => {
  assert.equal(manifest.id, '43');
  assert.equal(service.featureId, '43');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('43 invalid argument boundary', () => {
...
test('43 permission contract is explicit', () => {
...
test('43 scope and storage contract are bounded', () => {
...
test('43 dependency and renderer fallback are declared', () => {
...
test('43 redacts long metadata without changing command identity', () => {
...
test('43 failure matrix contains required acceptance paths', () => {
