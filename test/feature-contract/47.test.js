...
const service = serviceFor('47');
const contract = contractFor('47');

test('47 normal command contract', () => {
  assert.equal(manifest.id, '47');
  assert.equal(service.featureId, '47');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('47 invalid argument boundary', () => {
...
test('47 permission contract is explicit', () => {
...
test('47 scope and storage contract are bounded', () => {
...
test('47 dependency and renderer fallback are declared', () => {
...
test('47 redacts long metadata without changing command identity', () => {
...
test('47 failure matrix contains required acceptance paths', () => {
