...
const service = serviceFor('09');
const contract = contractFor('09');

test('09 normal command contract', () => {
  assert.equal(manifest.id, '09');
  assert.equal(service.featureId, '09');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('09 invalid argument boundary', () => {
...
test('09 permission contract is explicit', () => {
...
test('09 scope and storage contract are bounded', () => {
...
test('09 dependency and renderer fallback are declared', () => {
...
test('09 redacts long metadata without changing command identity', () => {
...
test('09 failure matrix contains required acceptance paths', () => {
