...
const service = serviceFor('25');
const contract = contractFor('25');

test('25 normal command contract', () => {
  assert.equal(manifest.id, '25');
  assert.equal(service.featureId, '25');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('25 invalid argument boundary', () => {
...
test('25 permission contract is explicit', () => {
...
test('25 scope and storage contract are bounded', () => {
...
test('25 dependency and renderer fallback are declared', () => {
...
test('25 redacts long metadata without changing command identity', () => {
...
test('25 failure matrix contains required acceptance paths', () => {
