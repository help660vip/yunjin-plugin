...
const service = serviceFor('50');
const contract = contractFor('50');

test('50 normal command contract', () => {
  assert.equal(manifest.id, '50');
  assert.equal(service.featureId, '50');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('50 invalid argument boundary', () => {
...
test('50 permission contract is explicit', () => {
...
test('50 scope and storage contract are bounded', () => {
...
test('50 dependency and renderer fallback are declared', () => {
...
test('50 redacts long metadata without changing command identity', () => {
...
test('50 failure matrix contains required acceptance paths', () => {
