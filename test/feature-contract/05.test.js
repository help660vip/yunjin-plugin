...
const service = serviceFor('05');
const contract = contractFor('05');

test('05 normal command contract', () => {
  assert.equal(manifest.id, '05');
  assert.equal(service.featureId, '05');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('05 invalid argument boundary', () => {
...
test('05 permission contract is explicit', () => {
...
test('05 scope and storage contract are bounded', () => {
...
test('05 dependency and renderer fallback are declared', () => {
...
test('05 redacts long metadata without changing command identity', () => {
...
test('05 failure matrix contains required acceptance paths', () => {
