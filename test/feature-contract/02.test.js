...
const service = serviceFor('02');
const contract = contractFor('02');

test('02 normal command contract', () => {
  assert.equal(manifest.id, '02');
  assert.equal(service.featureId, '02');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('02 invalid argument boundary', () => {
...
test('02 permission contract is explicit', () => {
...
test('02 scope and storage contract are bounded', () => {
...
test('02 dependency and renderer fallback are declared', () => {
...
test('02 redacts long metadata without changing command identity', () => {
...
test('02 failure matrix contains required acceptance paths', () => {
