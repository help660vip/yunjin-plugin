...
const service = serviceFor('03');
const contract = contractFor('03');

test('03 normal command contract', () => {
  assert.equal(manifest.id, '03');
  assert.equal(service.featureId, '03');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('03 invalid argument boundary', () => {
...
test('03 permission contract is explicit', () => {
...
test('03 scope and storage contract are bounded', () => {
...
test('03 dependency and renderer fallback are declared', () => {
...
test('03 redacts long metadata without changing command identity', () => {
...
test('03 failure matrix contains required acceptance paths', () => {
