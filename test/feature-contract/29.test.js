...
const service = serviceFor('29');
const contract = contractFor('29');

test('29 normal command contract', () => {
  assert.equal(manifest.id, '29');
  assert.equal(service.featureId, '29');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('29 invalid argument boundary', () => {
...
test('29 permission contract is explicit', () => {
...
test('29 scope and storage contract are bounded', () => {
...
test('29 dependency and renderer fallback are declared', () => {
...
test('29 redacts long metadata without changing command identity', () => {
...
test('29 failure matrix contains required acceptance paths', () => {
