...
const service = serviceFor('16');
const contract = contractFor('16');

test('16 normal command contract', () => {
  assert.equal(manifest.id, '16');
  assert.equal(service.featureId, '16');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('16 invalid argument boundary', () => {
...
test('16 permission contract is explicit', () => {
...
test('16 scope and storage contract are bounded', () => {
...
test('16 dependency and renderer fallback are declared', () => {
...
test('16 redacts long metadata without changing command identity', () => {
...
test('16 failure matrix contains required acceptance paths', () => {
