...
const service = serviceFor('45');
const contract = contractFor('45');

test('45 normal command contract', () => {
  assert.equal(manifest.id, '45');
  assert.equal(service.featureId, '45');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('45 invalid argument boundary', () => {
...
test('45 permission contract is explicit', () => {
...
test('45 scope and storage contract are bounded', () => {
...
test('45 dependency and renderer fallback are declared', () => {
...
test('45 redacts long metadata without changing command identity', () => {
...
test('45 failure matrix contains required acceptance paths', () => {
