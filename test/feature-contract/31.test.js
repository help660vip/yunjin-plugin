...
const service = serviceFor('31');
const contract = contractFor('31');

test('31 normal command contract', () => {
  assert.equal(manifest.id, '31');
  assert.equal(service.featureId, '31');
  assert.equal(service.command, manifest.command);
  assert.equal(service.validateInput(['�鿴']).ok, true);
});

test('31 invalid argument boundary', () => {
...
test('31 permission contract is explicit', () => {
...
test('31 scope and storage contract are bounded', () => {
...
test('31 dependency and renderer fallback are declared', () => {
...
test('31 redacts long metadata without changing command identity', () => {
...
test('31 failure matrix contains required acceptance paths', () => {
