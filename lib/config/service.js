...
const scopeAliases = Object.freeze({ global: 'global', '全局': 'global', group: 'group', '群': 'group', user: 'user', '用户': 'user' });
...
export class ConfigService {
  ...
  getEffective(scope = {}) {
    ...
    if (scopeInfo.groupId) Object.assign(values, this.state?.groups?.[scopeInfo.groupId] ?? {});
    if (scopeInfo.userId) Object.assign(values, this.state?.users?.[scopeInfo.userId] ?? {});
    return values;
  }
...
function plainObject(value) {
  ...
  return Object.fromEntries(Object.entries(value).filter(([key]) => !dangerousKeys.has(key)).map(([key, item]) => [String(key), item && typeof item === 'object' && !Array.isArray(item) ? plainObject(item) : item]));
}
...
function isSafeId(value) {
  ...
  return id.length > 0 && id.length <= 200 && !dangerousKeys.has(id);
}

function normalizeScope(scope = {}) {
  const input = typeof scope === 'string' ? { name: scope } : (scope || {});
  const name = scopeAliases[String(input.name ?? 'global').trim().toLowerCase()] || '';
  const rawId = input.id == null || input.id === '' ? null : String(input.id);
  const id = rawId && isSafeId(rawId) ? rawId : rawId === null ? null : '';
  const groupId = input.groupId == null ? (name === 'group' ? id : null) : String(input.groupId);
  const userId = input.userId == null ? (name === 'user' ? id : null) : String(input.userId);
  const valid = Boolean(name) && (rawId === null || Boolean(id));
  return { name, id, groupId: groupId && isSafeId(groupId) ? groupId : null, userId: userId && isSafeId(userId) ? userId : null, valid };
}
