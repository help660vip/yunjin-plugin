...
function normalizeTask(input, now, existing = {}) {
  ...
  const normalizedRunAt = runAt < now ? now : runAt;
  ...
  const stable = input.id || existing.id || stableId([featureId, scope, input.title || existing.title || '', normalizedRunAt, input.dedupeKey || existing.dedupeKey || '']);
  return {
    id: stable,
    featureId,
    title: String(input.title || existing.title || 'YunJin task').slice(0, 200),
    runAt: Number.isFinite(normalizedRunAt) ? normalizedRunAt : now,
    status: existing.id ? existing.status || input.status || 'scheduled' : 'scheduled',
    createdAt: existing.createdAt || input.createdAt || now,
    updatedAt: now,
    attempts: Number.isFinite(parsedAttempts) ? Math.floor(Math.max(0, parsedAttempts)) : 0,
    maxAttempts: Number.isFinite(parsedMaxAttempts) ? Math.floor(Math.max(1, parsedMaxAttempts)) : 1,
