...
export class NotificationBus {
  ...
  async publish(topic, payload = {}, options = {}) {
    ...
    if (options.dedupe !== false && this.onceKeys.has(key)) return { ok: true, duplicate: true, key };
    if (options.dedupe !== false) this.onceKeys.add(key);
    ...
    return { ok: results.every((item) => item.ok), duplicate: false, key, results };
  }
