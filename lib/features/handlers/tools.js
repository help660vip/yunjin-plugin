...
export async function handle32(manifest, event, args, runtime) {
  ...
  try {
    ...
    const title = String(data.title || query).slice(0, 128);
    const extract = String(data.extract || '\u6682\u65e0\u6458\u8981').slice(0, 2000);
    const source = String(data.content_urls?.desktop?.page || '').slice(0, 512);
    await ctx.audit('wiki.lookup', { length: query.length, found: Boolean(data.title) });
    return ctx.summary(title, [
      { label: '\u6458\u8981', value: extract },
      { label: '\u6765\u6e90', value: source }
    ]);
  } catch (error) {
    await ctx.audit('wiki.lookup.failed', { length: query.length, error: String(error?.message || error).slice(0, 160) });
...
export async function handle33(manifest, event, args, runtime) {
  const ctx = handlerContext(manifest, event, args, runtime);
  const usage = ctx.usage('<\u91d1\u989d> <\u6e90\u8d27\u5e01> <\u76ee\u6807\u8d27\u5e01>');
  const rawAmount = String(args[0] ?? '').trim();
  const amount = Number(rawAmount);
  const from = String(args[1] || 'CNY').toUpperCase();
  const to = String(args[2] || 'USD').toUpperCase();
  if (!rawAmount || !Number.isFinite(amount) || amount < 0 || amount > 1000000000000) return usage;
  if (!/^[A-Z]{3}$/u.test(from) || !/^[A-Z]{3}$/u.test(to)) return '\u8d27\u5e01\u4ee3\u7801\u5fc5\u987b\u662f 3 \u4f4d\u5b57\u6bcd\u3002';
  if (from === to) {
    await ctx.audit('exchange.local', { amount, from, to });
    return ctx.summary('\u6c47\u7387\u6362\u7b97', [{ label: '\u8f93\u5165', value: amount + ' ' + from }, { label: '\u7ed3\u679c', value: amount + ' ' + to }]);
  }
  try {
    ctx.network();
    const data = await fetchJson('https://api.frankfurter.app/latest?amount=' + encodeURIComponent(amount) + '&from=' + from + '&to=' + to, { hosts: ['frankfurter.app'], timeoutMs: 5000, attempts: 2, cacheTtlMs: 300000, cacheStaleMs: 600000 });
    const result = Number(data?.rates?.[to]);
    if (!Number.isFinite(result)) throw new Error('invalid exchange response');
    await ctx.audit('exchange.rate', { amount, from, to });
    return ctx.summary('\u6c47\u7387\u6362\u7b97', [{ label: '\u8f93\u5165', value: amount + ' ' + from }, { label: '\u7ed3\u679c', value: result + ' ' + to }]);
  } catch (error) {
    await ctx.audit('exchange.failed', { amount, from, to, error: String(error?.message || error).slice(0, 160) });
...
export async function handle34(manifest, event, args, runtime) {
  ...
  const usage = ctx.usage('添加 <内容> | 完成 <待办 ID>');
  const action = String(args[0] || '').toLowerCase();
  if (action === '添加' || action === 'add') {
    ...
    if (!value || value.length > 500 || hasControl) return usage;
    try {
      const item = await ctx.add({ text: value, done: false }, 'items');
      await ctx.audit('todo.add', { itemId: item.id, length: value.length });
      return '待办已添加：' + item.id;
    } catch (error) {
      await ctx.audit('todo.failed', { action: 'add', error: String(error?.message || error).slice(0, 160) });
      return '待办服务不可用：' + publicErrorMessage(error);
    }
  }
  if (action === '完成' || action === 'done') {
    ...
    try {
      ...
      if (item.done) return '待办已完成：' + id;
      item.done = true;
      item.completedAt = ctx.now;
      await ctx.update((next) => { next.items = state.items; return item; }, { items: [] });
      await ctx.audit('todo.done', { itemId: id });
      return '待办已完成：' + id;
    } catch (error) {
      await ctx.audit('todo.failed', { action: 'done', error: String(error?.message || error).slice(0, 160) });
      return '待办服务不可用：' + publicErrorMessage(error);
    }
  }
  try {
    const items = (await ctx.store.list('items')).filter((item) => !item.done);
    await ctx.audit('todo.list', { count: items.length });
    return listMessage(ctx, items, { usage: '添加 <内容> | 完成 <待办 ID>' });
  } catch (error) {
    await ctx.audit('todo.failed', { action: 'list', error: String(error?.message || error).slice(0, 160) });
    return '待办服务不可用：' + publicErrorMessage(error);
  }
}
...
export async function handle35(manifest, event, args, runtime) {
  const ctx = handlerContext(manifest, event, args, runtime);
  const configured = runtime.config?.getGlobal?.('core.timezone') || ctx.runtime.core.clock.timeZone || 'Asia/Shanghai';
  const timeZone = String(args[0] || configured).trim();
  const hasControl = [...timeZone].some((character) => {
    const code = character.codePointAt(0);
    return code < 32 && ![9, 10, 13].includes(code) || code === 127;
  });
  if (!timeZone || timeZone.length > 64 || hasControl) return ctx.usage('[IANA \u65f6\u533a]');
  try {
    const value = ctx.runtime.core.clock.format(ctx.now, { timeZone });
    await ctx.audit('clock.lookup', { timeZone });
    return ctx.summary('\u5f53\u524d\u65f6\u95f4', [{ label: '\u65f6\u533a', value: timeZone }, { label: '\u65f6\u95f4', value }, { label: '\u65f6\u95f4\u6233', value: String(ctx.now) }]);
  } catch (error) {
    await ctx.audit('clock.failed', { timeZone, error: String(error?.message || error).slice(0, 160) });
