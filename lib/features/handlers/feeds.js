...
export async function handle18(manifest, event, args, runtime) {
  ...
  const normalizeTarget = (value) => {
    ...
    return canonicalHttpUrl(normalized) || normalized;
  };
...
export async function handle19(manifest, event, args, runtime) {
  ...
  if (action === '\u6dfb\u52a0' || action === 'add' || action === '\u8ba2\u9605' || action === 'subscribe') {
    ...
    const canonicalUrl = canonicalHttpUrl(url.href) || url.href;
    const existing = (await ctx.store.list('feeds')).find((item) => item.url === canonicalUrl);
    const item = existing || await ctx.add({ url: canonicalUrl, title: '', lastHash: '' }, 'feeds', { uniqueBy: 'url' });
...
export async function handle22(manifest, event, args, runtime) {
  const ctx = handlerContext(manifest, event, args, runtime, { level: 'group' });
  if (!event?.groupId || event.groupId === 'private') return '\u53ea\u80fd\u5728\u7fa4\u804a\u4e2d\u4f7f\u7528\u8be5\u529f\u80fd\u3002';
  const usage = ctx.usage('\u751f\u6210');
  if (args.length && !['\u751f\u6210', 'generate'].includes(String(args[0] || '').toLowerCase())) return usage;
  if (args.length > 1) return usage;
  let state;
  try {
    state = await ctx.store.read({ messageCount: 0, commandCount: 0, reports: [] });
  } catch (error) {
    await ctx.audit('report.generate.failed', { error: String(error?.message || error).slice(0, 160) });
    return '\u62a5\u544a\u6570\u636e\u4e0d\u53ef\u7528\uff0c\u8bf7\u7a0d\u540e\u91cd\u8bd5\u3002';
  }
  const report = { id: ctx.newId(), createdAt: ctx.now, scope: ctx.groupId, messages: Number(state.messageCount || 0), commands: Number(state.commandCount || 0), records: Number(state.reports?.length || 0) };
  try {
    await ctx.update((next) => { next.reports = [report, ...(next.reports || [])].slice(0, 50); return report; }, { reports: [] });
  } catch (error) {
    await ctx.audit('report.generate.failed', { reportId: report.id, error: String(error?.message || error).slice(0, 160) });
    return '\u62a5\u544a\u4fdd\u5b58\u5931\u8d25\uff0c\u5df2\u4fdd\u7559\u672c\u6b21\u7ed3\u679c\u3002';
  }
  await ctx.audit('report.generate', { reportId: report.id, scope: report.scope });
  return ctx.summary('\u7fa4\u7ec4\u62a5\u544a', [
    { label: '\u4f5c\u7528\u57df', value: report.scope },
    { label: '\u6d88\u606f\u6570', value: report.messages },
...
export async function handle23(manifest, event, args, runtime) {
  ...
  if (!event?.groupId || event.groupId === 'private') return '\u53ea\u80fd\u5728\u7fa4\u804a\u4e2d\u4f7f\u7528\u8be5\u529f\u80fd\u3002';
  const action = String(args[0] || '').toLowerCase();
  const usageRetry = ctx.usage('\u91cd\u8bd5 <\u63a8\u9001 ID>');
  const usageList = ctx.usage('\u5217\u8868');
  if (action === '\u91cd\u8bd5' || action === 'retry') {
    if (args.length !== 2) return usageRetry;
    const id = required(args[1], usageRetry);
    let state;
    try {
      state = await ctx.store.read({ items: [] });
    } catch (error) {
      await ctx.audit('push.retry.failed', { pushId: id, error: String(error?.message || error).slice(0, 160) });
      return '\u63a8\u9001\u8bb0\u5f55\u4e0d\u53ef\u7528\uff0c\u65e0\u6cd5\u91cd\u8bd5\u3002';
    }
    const item = state.items?.find((entry) => entry.id === id && (!entry.groupId || entry.groupId === ctx.groupId) && (!entry.botId || entry.botId === ctx.botId) && (!entry.payload?.groupId || entry.payload.groupId === ctx.groupId) && (!entry.payload?.botId || entry.payload.botId === ctx.botId));
    if (!item) return '\u672a\u627e\u5230\u63a8\u9001\u8bb0\u5f55\u3002';
    const retryCount = Number(item.retryCount || 0);
    if (retryCount >= 5) {
      await ctx.audit('push.retry.denied', { pushId: id, reason: 'retry-limit' });
      return '\u8be5\u63a8\u9001\u5df2\u8fbe\u5230\u91cd\u8bd5\u4e0a\u9650\u3002';
    }
    if (item.status === 'sent' || item.status === 'success') return '\u8be5\u63a8\u9001\u5df2\u6210\u529f\uff0c\u65e0\u9700\u91cd\u8bd5\u3002';
    if (!runtime.notifications?.publish) {
      await ctx.audit('push.retry.denied', { pushId: id, reason: 'notification-unavailable' });
      return '\u901a\u77e5\u670d\u52a1\u4e0d\u53ef\u7528\uff0c\u65e0\u6cd5\u91cd\u8bd5\u3002';
    }
    let result;
    try {
      result = await runtime.notifications.publish('task.execute', {
        taskId: item.taskId,
    ...
    try {
      await ctx.update((next) => {
        ...
        if (current) {
          ...
          if (!result.ok) current.error = String(result.error || result.results?.find((entry) => !entry.ok)?.error || 'notification delivery failed').slice(0, 300);
        }
        next.items = items;
        return current;
      }, { items: [] });
    } catch (error) {
      await ctx.audit('push.retry.failed', { pushId: id, delivered: Boolean(result.ok), error: String(error?.message || error).slice(0, 160) });
      return result.ok ? '\u63a8\u9001\u5df2\u53d1\u51fa\uff0c\u4f46\u72b6\u6001\u4fdd\u5b58\u5931\u8d25\u3002' : '\u63a8\u9001\u91cd\u8bd5\u5931\u8d25\uff0c\u72b6\u6001\u4fdd\u5b58\u5931\u8d25\u3002';
    }
    await ctx.audit(result.ok ? 'push.retry' : 'push.retry.failed', { pushId: id, retryCount: retryCount + 1 });
    return result.ok ? '\u63a8\u9001\u91cd\u8bd5\u6210\u529f\uff1a' + id : '\u63a8\u9001\u91cd\u8bd5\u5931\u8d25\uff1a' + id;
  }
  if (action === '\u5217\u8868' || action === 'list' || !args.length) {
    if (args.length > 1) return usageList;
    await ctx.audit('push.list', {});
    return listState(ctx, 'items', usageRetry);
  }
  return usageList;
}
...
export async function handle24(manifest, event, args, runtime) {
  ...
  const action = String(args[0] || '').toLowerCase();
  const usageAdd = ctx.usage('\u6dfb\u52a0 <\u4fe1\u606f\u6e90 URL>');
  const usageRefresh = ctx.usage('\u5237\u65b0');
  const usageList = ctx.usage('\u5217\u8868');
  let state;
  try {
    state = await ctx.store.read({ sources: [] });
  } catch (error) {
    await ctx.audit('source.read.failed', { error: String(error?.message || error).slice(0, 160) });
    return '\u4fe1\u606f\u6e90\u6570\u636e\u4e0d\u53ef\u7528\uff0c\u8bf7\u7a0d\u540e\u91cd\u8bd5\u3002';
  }
  if (action === '\u6dfb\u52a0' || action === 'add') {
    if (args.length !== 2) return usageAdd;
    const url = required(args[1], usageAdd);
    let target;
    try {
      target = validateUrl(url);
    } catch {
      return '\u4fe1\u606f\u6e90 URL \u4e0d\u5b89\u5168\u6216\u4e0d\u53ef\u8bbf\u95ee\u3002';
    }
    const item = await ctx.add({ url: target.href, enabled: true }, 'sources', { uniqueBy: 'url' });
    await ctx.audit('source.add', { sourceId: item.id, duplicate: (state.sources || []).some((entry) => entry.url === target.href) });
    return '\u4fe1\u606f\u6e90\u5df2\u6dfb\u52a0\uff1a' + item.id;
  }
  if (action === '\u5237\u65b0' || action === 'refresh') {
    if (args.length !== 1) return usageRefresh;
    if (typeof ctx.network !== 'function') return '\u7f51\u7edc\u670d\u52a1\u4e0d\u53ef\u7528\uff0c\u5df2\u4fdd\u7559\u4fe1\u606f\u6e90\u914d\u7f6e\u3002';
    try {
      ctx.network();
    } catch {
      await ctx.audit('source.refresh.failed', { reason: 'network-unavailable' });
      return '\u7f51\u7edc\u670d\u52a1\u4e0d\u53ef\u7528\uff0c\u5df2\u4fdd\u7559\u4fe1\u606f\u6e90\u914d\u7f6e\u3002';
    }
    const output = [];
    let failed = 0;
    for (const source of (state.sources || []).filter((entry) => entry.enabled !== false).slice(0, 10)) {
      try {
        const raw = await fetchText(source.url, { maxBytes: 128 * 1024, timeoutMs: 4000, attempts: 1, cacheTtlMs: 300000, cacheStaleMs: 600000 });
        const title = raw.match(/<title[^>]*>([\s\S]*?)<\/title>/iu)?.[1]?.replace(/<[^>]*>/gu, '').replace(/\s+/gu, ' ').trim().slice(0, 200);
        if (title) output.push(title);
      } catch {
        failed += 1;
      }
    }
    await ctx.audit('source.refresh', { sources: output.length, failed });
    return output.length ? output.join('\\n') : '\u6ca1\u6709\u53ef\u7528\u4fe1\u606f\u6e90\uff0c\u5df2\u4fdd\u7559\u6587\u672c\u964d\u7ea7\u3002';
  }
  if (action === '\u5217\u8868' || action === 'list' || !args.length) {
    if (args.length > 1) return usageList;
    await ctx.audit('source.list', { count: (state.sources || []).length });
    return listState(ctx, 'sources', usageAdd + ' | ' + usageRefresh);
  }
  return usageList;
}
...
export async function handle25(manifest, event, args, runtime) {
  ...
  const action = String(args[0] || '').toLowerCase();
  const usageAdd = ctx.usage('\u6dfb\u52a0 <Git URL>');
  const usageDelete = ctx.usage('\u5220\u9664 <\u76ee\u6807 ID>');
  const usageCheck = ctx.usage('\u68c0\u67e5 [\u76ee\u6807 ID]');
  const usageList = ctx.usage('\u5217\u8868');
  const scheduler = runtime.scheduler;
  ...
  if (action === '\u6dfb\u52a0' || action === 'add') {
    ...
    try {
      ...
      return 'Git URL \u4e0d\u5b89\u5168\u6216\u4e0d\u53ef\u8bbf\u95ee\u3002';
    }
    const state = await ctx.store.read({ repositories: [] });
    const existing = (state.repositories || []).find((entry) => entry.url === target.href);
    const item = existing || await ctx.add({ url: target.href, lastHash: '', lastCheckedAt: 0 }, 'repositories', { uniqueBy: 'url' });
    if (!scheduler?.list || !scheduler?.create) {
      await ctx.audit('git.add', { repositoryId: item.id, scheduler: 'unavailable', created: !existing });
      return '\u8f6e\u8be2\u76ee\u6807\u5df2\u4fdd\u5b58\uff1a' + item.id + '\\n\u8c03\u5ea6\u670d\u52a1\u4e0d\u53ef\u7528\uff0c\u8bf7\u4f7f\u7528\u68c0\u67e5\u64cd\u4f5c\u3002';
    }
    const activeTask = (await scheduler.list()).find((entry) => entry.featureId === ctx.id && entry.payload?.repoId === item.id && ['scheduled', 'running'].includes(entry.status));
    if (activeTask) {
      await ctx.audit('git.add', { repositoryId: item.id, taskId: activeTask.id, duplicate: true });
      return '\u8f6e\u8be2\u5df2\u5b58\u5728\uff1a' + item.id + '\\n\u4efb\u52a1\uff1a' + activeTask.id;
    }
    let task;
    try {
      task = await scheduler.create({ featureId: ctx.id, title: 'Git ' + target.href, delayMs: 30 * 60 * 1000, repeatMs: 30 * 60 * 1000, dedupeKey: 'git:' + ctx.botId + ':' + item.id, groupId: ctx.groupId, userId: ctx.userId, botId: ctx.botId, payload: { kind: 'git', repoId: item.id, target: target.href, groupId: ctx.groupId, userId: ctx.userId, botId: ctx.botId } });
    } catch (error) {
      if (!existing) await ctx.store.remove((entry) => entry.id === item.id, 'repositories');
      await ctx.audit('git.add.failed', { repositoryId: item.id, rolledBack: !existing, error: String(error?.message || error).slice(0, 160) });
      return 'Git \u8f6e\u8be2\u4efb\u52a1\u521b\u5efa\u5931\u8d25\uff0c\u5df2\u56de\u6eda\u672c\u6b21\u65b0\u589e\u3002';
    }
    await ctx.audit('git.add', { repositoryId: item.id, taskId: task.id, created: !existing });
  ...
  if (action === '\u5220\u9664' || action === 'del') {
    ...
    if (scheduler?.list && scheduler?.cancel) {
      const tasks = await scheduler.list();
      const results = await Promise.all(tasks.filter((task) => task.featureId === ctx.id && task.payload?.repoId === id && ['scheduled', 'running'].includes(task.status)).map((task) => scheduler.cancel(task.id).catch(() => false)));
      cancelled = results.filter(Boolean).length;
    }
    await ctx.audit('git.delete', { repositoryId: id, cancelled });
    return '\u8f6e\u8be2\u76ee\u6807\u5df2\u5220\u9664\u3002';
  }
  if (action === '\u68c0\u67e5' || action === 'check') {
    if (args.length > 2) return usageCheck;
    const state = await ctx.store.read({ repositories: [] });
    const item = args[1] ? state.repositories?.find((entry) => entry.id === args[1]) : state.repositories?.[0];
    if (!item) return usageCheck;
    try {
      ctx.network();
      const raw = await fetchText(item.url, { maxBytes: 128 * 1024, timeoutMs: 5000, attempts: 2, cache: false });
      const nextHash = hash(raw);
      const changed = Boolean(item.lastHash && item.lastHash !== nextHash);
      const nextItem = { ...item, lastHash: nextHash, lastCheckedAt: ctx.now };
      await ctx.update((next) => {
        next.repositories = (next.repositories || []).map((entry) => entry.id === item.id ? nextItem : entry);
        return nextItem;
      }, { repositories: [] });
      await ctx.audit('git.check', { repositoryId: item.id, changed });
      return '\u68c0\u67e5\u5b8c\u6210\uff1a' + (changed ? '\u53d1\u73b0\u5185\u5bb9\u53d8\u5316\u3002' : '\u65e0\u53d8\u5316\u3002');
    } catch (error) {
      await ctx.audit('git.check.failed', { repositoryId: item.id, error: String(error?.message || error).slice(0, 160) });
      return 'Git \u8f6e\u8be2\u5931\u8d25\uff1a' + publicErrorMessage(error);
    }
  }
  if (action === '\u5217\u8868' || action === 'list' || !args.length) {
    if (args.length > 1) return usageList;
    await ctx.audit('git.list', { scheduler: Boolean(scheduler?.list) });
    return listState(ctx, 'repositories', usageAdd + ' | ' + usageCheck + ' | ' + usageDelete);
  }
  return usageList;
}
...
export async function handle26(manifest, event, args, runtime) {
  const ctx = handlerContext(manifest, event, args, runtime);
  const usage = ctx.usage('<\u57ce\u5e02>');
  const city = required(args.join(' '), usage, '\u8bf7\u63d0\u4f9b\u57ce\u5e02\u3002');
  const hasControl = [...city].some((character) => {
    const code = character.codePointAt(0);
    return code < 32 || code === 127;
  });
  if (city.length > 64 || hasControl) return usage;
  try {
    ctx.network();
    ...
    if (!place) {
      await ctx.audit('weather.lookup', { city, found: false });
      return '\u672a\u627e\u5230\u57ce\u5e02\u3002';
    }
    ...
    await ctx.audit('weather.lookup', { city, found: true, place: String(place.name || city).slice(0, 64) });
    ...
    await ctx.audit('weather.lookup.failed', { city, error: String(error?.message || error).slice(0, 160) });
