...
export async function handle02(manifest, event, args, runtime) {
  const ctx = handlerContext(manifest, event, args, runtime);
  const value = required(ctx.value, ctx.usage('记录 <异常描述>'), '请提供异常描述。');
  if (actionIs(ctx, '列表', 'list', '查看', 'view') && !ctx.value) return listState(ctx, 'items', '记录 <异常描述>');
  const item = await ctx.add({ text: value, type: 'error', fingerprint: ctx.stable(value) });
...
export async function handle08(manifest, event, args, runtime) {
  ...
  const scopeName = args[1] || 'global';
  const key = required(args[2], ctx.usage('获取 <作用域> <键> | 设置 <作用域> <键> <JSON>'));
  const scope = { name: scopeName, id: args[3] };
  if (action === '获取' || action === 'get') return runtime.config.getEffectiveValue(scope, key);
  if (action === '设置' || action === 'set') {
    if (!canWriteConfig(event, scope)) return '只有 Yunzai OP 可以修改全局配置；群配置需要本群管理员。';
    let value;
    try { value = JSON.parse(args.slice(3).join(' ')); } catch { return '配置值必须是 JSON。'; }
    const result = await runtime.config.set(scope, key, value);
    await ctx.audit('config.set', { key, scope: scopeName });
    return result;
  }
  return ctx.usage('查看 | 获取 <作用域> <键> | 设置 <作用域> <键> <JSON> | 重载 | 校验');
}
