...
export async function handle07(manifest, event, args, runtime) {
  ...
  const searchArgs = viewAlias ? args.slice(1) : args;
  if (searchArgs.length > 2) return ctx.usage('\u67e5\u770b [core|governance|feeds|tools|media|community|\u5173\u952e\u8bcd]');
  const requested = searchArgs.join(' ').trim().toLowerCase();
  const areas = ['core', 'governance', 'feeds', 'tools', 'media', 'community'];
  const labels = { core: '\u57fa\u7840\u8fd0\u884c', governance: '\u6743\u9650\u7ba1\u7406', feeds: '\u8ba2\u9605\u63a8\u9001', tools: '\u65e5\u5e38\u5de5\u5177', media: '\u5185\u5bb9\u5a92\u4f53', community: '\u793e\u533a\u4e92\u52a8' };
  const area = areas.includes(requested) ? requested : '';
  const query = area ? '' : requested;
  let features = [...runtime.registry.list(event)].sort((left, right) => String(left.id).localeCompare(String(right.id), 'en', { numeric: true }));
  if (area) features = features.filter((item) => item.area === area);
  if (query) features = features.filter((item) => [item.id, item.command, item.name, item.area].some((value) => String(value || '').toLowerCase().includes(query)));
  if (!features.length) {
    await ctx.audit('help.empty', { area: area || 'all', query: query || requested || 'all' });
    if (query && /^[a-z0-9_-]+$/u.test(query)) return ctx.usage('\u67e5\u770b [core|governance|feeds|tools|media|community|\u5173\u952e\u8bcd]');
    return query ? '\u6ca1\u6709\u627e\u5230\u5339\u914d\u201c' + query + '\u201d\u7684\u80fd\u529b\uff0c\u8bf7\u53d1\u9001 #\u4e91\u9526\u5e2e\u52a9 \u67e5\u770b\u5168\u90e8\u529f\u80fd\u3002' : '\u6682\u65e0\u53ef\u7528\u80fd\u529b\u3002';
  }
  const groups = area ? [area] : areas;
  const title = query ? '\u4e91\u9526\u5e2e\u52a9\uff08\u7b5b\u9009\uff1a' + query + '\uff09' : '\u4e91\u9526\u5e2e\u52a9';
  const rows = [title];
  for (const group of groups) {
    const entries = features.filter((item) => item.area === group);
    if (!entries.length) continue;
    rows.push('', '## ' + labels[group], ...entries.map((item) => String.fromCodePoint(0x23, 0x4e91, 0x9526) + item.command + '  ' + item.name + ' [' + item.access + ']'));
  }
  await ctx.audit('help.view', { area: area || 'all', query: query || 'all', count: features.length });
  return rows.join('\n');
}
...
export async function handle17(manifest, event, args, runtime) {
  const ctx = handlerContext(manifest, event, args, runtime, { level: 'group' });
  if (!ctx.groupId || ctx.groupId === 'private') return '\u6b64\u80fd\u529b\u53ea\u80fd\u5728\u7fa4\u804a\u4f7f\u7528\u3002';
  const confirmTokens = new Set(['\u786e\u8ba4', 'confirm']);
  const actionTokens = new Set(['\u64a4\u56de', 'withdraw']);
  const raw = args.flatMap((item) => String(item).split(',')).map((item) => item.trim()).filter(Boolean);
  const hasConfirm = raw.some((item) => confirmTokens.has(item.toLowerCase()));
  const candidates = raw.filter((item) => !confirmTokens.has(item.toLowerCase()) && !actionTokens.has(item.toLowerCase()));
  if (!hasConfirm) return ctx.usage('\u64a4\u56de <\u6d88\u606f ID> [\u6d88\u606f ID...] \u786e\u8ba4');
  if (!candidates.length || candidates.length > 20 || candidates.some((item) => !/^\w[\w:-]{0,100}$/u.test(item))) return ctx.usage('\u64a4\u56de <\u6d88\u606f ID> [\u6d88\u606f ID...] \u786e\u8ba4');
  ...
  for (const messageId of ids) {
    try {
      ...
      await ctx.audit('withdraw.error', { messageId, error: publicErrorMessage(error) });
    }
  }
