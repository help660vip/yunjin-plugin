...
import { renderCard, cardView } from '../../renderer/service.js';
...
export function handlerContext(manifest, event, args, runtime, options = {}) {
  ...
  const store = featureStore(runtime, id, event, { level: options.level || (event?.groupId && event.groupId !== 'private' ? 'group' : 'user'), maxItems: options.maxItems });
  return {
    id,
    manifest,
    contract,
    ...
    async audit(action, details = {}) {
      return runtime.audit.record({ action, ...details, featureId: id, userId: this.userId, groupId: this.groupId, botId: this.botId });
    },
    network() {
    ...
    async add(item, field = 'items', options = {}) {
      ...
      return store.add({ ...value, id: value.id || randomUUID(), createdAt: value.createdAt || this.now, userId: this.userId, groupId: this.groupId, botId: this.botId }, field, options);
    },
    ...
    async summary(title, rows = [], options = {}) {
      ...
      if (options.render) {
        const budget = this.render();
        if (budget?.allowed !== false) {
          const viewModel = options.viewModel || cardView(this.id, title, rows, {
            view: options.view || this.contract.view || 'card'
          });
          return renderCard(this.event, {
...
export function listMessage(ctx, values, options = {}) {
  ...
  const offset = Number.isFinite(result.offset) ? result.offset : Math.max(0, (Number(result.page) - 1) * (Number(result.size || options.size || 20)));
  const lines = result.items.map((item, index) => {
    const value = item?.text || item?.value || item?.trigger || item?.word || item?.query || item?.title || item?.name || item?.url || item?.id || JSON.stringify(item);
    return (offset + index + 1) + '. ' + cleanText(value, { max: 500 });
  });
...
export function parseScope(value, event) {
  ...
  return { level: event?.groupId && event.groupId !== 'private' ? 'group' : 'user' };
}
