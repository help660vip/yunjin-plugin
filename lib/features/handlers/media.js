...
export async function handle40(manifest, event, args, runtime) {
  const ctx = handlerContext(manifest, event, args, runtime, { level: 'user' });
  const usage = ctx.usage('记录 [媒体 URL] | 列表');
  const action = String(args[0] || '').toLowerCase();
  if (action === '列表' || action === 'list') {
    try {
      const records = await ctx.store.list('media');
      await ctx.audit('media.list', { count: records.length });
      return listMessage(ctx, records, { usage: '记录 [媒体 URL] | 列表', max: 100 });
    } catch (error) {
      await ctx.audit('media.failed', { action: 'list', error: String(error?.message || error).slice(0, 160) });
      return '媒体引用存储不可用：' + publicErrorMessage(error);
    }
  }
  const explicitRecord = action === '记录' || action === 'record';
  const segments = (event.segments || []).filter((segment) => ['audio', 'video', 'file'].includes(segment.type));
  const values = segments.length ? segments : (explicitRecord ? args.slice(1).map((url) => ({ type: 'reference', url })) : args.map((url) => ({ type: 'reference', url })));
  if (!values.length || values.length > 10) return usage;
  const records = [];
  for (const segment of values) {
    const rawUrl = String(segment.url || segment.file || '').trim();
    if (!rawUrl || rawUrl.length > 2048) continue;
    let url;
    try { url = validateUrl(rawUrl).href; } catch { continue; }
    records.push({ type: cleanText(segment.type || 'reference', { max: 32 }), url: cleanText(url, { max: 2048 }), name: cleanText(segment.name || '', { max: 200 }) });
  }
  if (!records.length) return '没有可记录的安全媒体引用。';
  try {
    for (const record of records) await ctx.add(record, 'media', { uniqueBy: 'url' });
    await ctx.audit('media.record', { count: records.length, types: [...new Set(records.map((item) => item.type))] });
    return '媒体引用已记录：' + records.length + ' 条。';
  } catch (error) {
    await ctx.audit('media.failed', { action: 'record', count: records.length, error: String(error?.message || error).slice(0, 160) });
    return '媒体引用存储不可用：' + publicErrorMessage(error);
  }
}
...
export async function handle41(manifest, event, args, runtime) {
  ...
  const usage = ctx.usage('添加 <触发词> <回复> | 删除 <触发词> | 列表');
  const action = String(args[0] || '').toLowerCase();
  if (action === '添加' || action === 'add') {
    const trigger = String(args[1] || '').trim();
    const response = String(args.slice(2).join(' ') || '').trim();
    const hasControl = (input) => [...input].some((character) => { const code = character.codePointAt(0); return code < 32 && ![9, 10, 13].includes(code) || code === 127; });
    if (!trigger || !response || trigger.length > 100 || response.length > 1000 || hasControl(trigger) || hasControl(response)) return usage;
    try {
      ...
      await ctx.audit('autoreply.add', { itemId: item.id, triggerLength: trigger.length, responseLength: response.length });
      return '自动回复规则已添加：' + item.id;
    } catch (error) {
      await ctx.audit('autoreply.failed', { action: 'add', error: String(error?.message || error).slice(0, 160) });
      return '自动回复存储不可用：' + publicErrorMessage(error);
    }
  }
  if (action === '删除' || action === 'del' || action === 'delete') {
    const trigger = String(args[1] || '').trim();
    if (!trigger || trigger.length > 100) return usage;
    try {
      const removed = await ctx.store.remove((item) => item.trigger === trigger, 'rules');
      await ctx.audit('autoreply.delete', { triggerLength: trigger.length, removed: Boolean(removed) });
      return removed ? '自动回复规则已删除。' : '未找到自动回复规则。';
    } catch (error) {
      await ctx.audit('autoreply.failed', { action: 'delete', error: String(error?.message || error).slice(0, 160) });
      return '自动回复存储不可用：' + publicErrorMessage(error);
    }
  }
  if (action && action !== '列表' && action !== 'list') return usage;
  try {
    const rules = await ctx.store.list('rules');
    await ctx.audit('autoreply.list', { count: rules.length });
    return listMessage(ctx, rules, { usage: '添加 <触发词> <回复> | 删除 <触发词> | 列表', max: 100 });
  } catch (error) {
    await ctx.audit('autoreply.failed', { action: 'list', error: String(error?.message || error).slice(0, 160) });
    return '自动回复存储不可用：' + publicErrorMessage(error);
  }
}
...
export async function handle42(manifest, event, args, runtime) {
  const ctx = handlerContext(manifest, event, args, runtime, { level: 'group' });
  const usage = ctx.usage('添加 <词语> | 删除 <词语> | 列表');
  const action = String(args[0] || '').toLowerCase();
  if (action === '添加' || action === 'add') {
    const word = String(args.slice(1).join(' ') || '').trim();
    const hasControl = [...word].some((character) => { const code = character.codePointAt(0); return code < 32 && ![9, 10, 13].includes(code) || code === 127; });
    if (!word || word.length > 200 || hasControl) return usage;
    try {
      const item = await ctx.add({ word: cleanText(word, { max: 200 }) }, 'words', { uniqueBy: 'word' });
      await ctx.audit('wordbank.add', { itemId: item.id, length: word.length });
      return '词库已添加：' + item.id;
    } catch (error) {
      await ctx.audit('wordbank.failed', { action: 'add', error: String(error?.message || error).slice(0, 160) });
      return '词库存储不可用：' + publicErrorMessage(error);
    }
  }
  if (action === '删除' || action === 'del' || action === 'delete') {
    const word = String(args.slice(1).join(' ') || '').trim();
    if (!word || word.length > 200) return usage;
    try {
      const removed = await ctx.store.remove((item) => item.word === word, 'words');
      await ctx.audit('wordbank.delete', { length: word.length, removed: Boolean(removed) });
      return removed ? '词库项已删除。' : '未找到词库项。';
    } catch (error) {
      await ctx.audit('wordbank.failed', { action: 'delete', error: String(error?.message || error).slice(0, 160) });
      return '词库存储不可用：' + publicErrorMessage(error);
    }
  }
  if (action && action !== '列表' && action !== 'list') return usage;
  try {
    const words = await ctx.store.list('words');
    await ctx.audit('wordbank.list', { count: words.length });
    return listMessage(ctx, words, { usage: '添加 <词语> | 删除 <词语> | 列表', max: 200 });
  } catch (error) {
    await ctx.audit('wordbank.failed', { action: 'list', error: String(error?.message || error).slice(0, 160) });
    return '词库存储不可用：' + publicErrorMessage(error);
  }
}
...
export async function handle43(manifest, event, args, runtime) {
  const ctx = handlerContext(manifest, event, args, runtime, { level: 'group' });
  const usage = ctx.usage('生成 | 清理');
  const action = String(args[0] || '').toLowerCase();
  if (action === '清理' || action === 'clear') {
    try {
      await ctx.store.clear();
      await ctx.audit('summary.clear', {});
      return '群摘要数据已清理。';
    } catch (error) {
      await ctx.audit('summary.failed', { action: 'clear', error: String(error?.message || error).slice(0, 160) });
      return '群摘要存储不可用：' + publicErrorMessage(error);
    }
  }
  if (action && action !== '生成' && action !== 'generate' && action !== '总结' && action !== 'summary') return usage;
  try {
    ...
    const recent = messages.slice(-5).map((item) => cleanText(item?.text || item?.value || '', { max: 200 })).filter(Boolean).join(' | ').slice(0, 1000) || '暂无';
    const messageCount = Number.isFinite(Number(counters.messages)) ? Number(counters.messages) : messages.length;
    const commandCount = Number.isFinite(Number(counters.commands)) ? Number(counters.commands) : 0;
    const rows = [
      { label: '群', value: ctx.groupId || '私聊' },
      { label: '消息计数', value: String(Math.max(0, Math.min(messageCount, 1000000000))) },
      { label: '命令计数', value: String(Math.max(0, Math.min(commandCount, 1000000000))) },
      { label: '最近记录', value: recent }
    ];
    const result = await ctx.summary('群摘要', rows, { render: true });
    await ctx.audit('summary.generate', { messageCount, commandCount, rendered: typeof result !== 'string' });
    return result;
  } catch (error) {
    await ctx.audit('summary.failed', { action: 'generate', error: String(error?.message || error).slice(0, 160) });
    return '群摘要生成失败：' + publicErrorMessage(error);
  }
}
