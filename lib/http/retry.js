...
export async function withRetry(operation, options = {}) {
  const attempts = normalizeAttempts(options.attempts ?? 1);
  const baseMs = normalizeDelay(options.baseMs ?? 100, 100);
  const maxMs = Math.max(baseMs, normalizeDelay(options.maxMs ?? 2000, 2000));
