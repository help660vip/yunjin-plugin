...
export class YunjinError extends Error {
  ...
  toJSON() {
    return {
      name: this.name,
      code: this.code,
      message: this.expose ? this.message : zh(25805, 20316, 22833, 36133),
      status: this.status,
...
export function notAuthorized(message = zh(26435, 38480, 19981, 36275), details) {
  return new YunjinError(ERROR_CODES.NOT_AUTHORIZED, message, { status: 403, details });
}

export function featureDisabled(featureId) {
  return new YunjinError(ERROR_CODES.FEATURE_DISABLED, zh(33021, 21147, 24050, 20572, 29992), { status: 404, featureId });
}

export function rateLimited(retryAfterMs, details) {
  return new YunjinError(ERROR_CODES.RATE_LIMITED, zh(25805, 20316, 36807, 20110, 39057, 32321, 65292, 35831, 31245, 21518, 20877, 35797), {
    status: 429,
    retryable: true,
...
export function quotaExceeded(details) {
  return new YunjinError(ERROR_CODES.QUOTA_EXCEEDED, zh(37197, 39069, 24050, 29992, 23613), { status: 429, details });
}

export function dependencyMissing(name) {
  return new YunjinError(ERROR_CODES.DEPENDENCY_MISSING, zh(32570, 23569, 20381, 36182, 65306) + safeText(name, zh(26410, 30693, 20381, 36182), 120), { status: 503 });
}
...
export function networkTimeout(url, details) {
  return new YunjinError(ERROR_CODES.NETWORK_TIMEOUT, zh(32593, 32476, 35831, 27714, 36229, 26102), { status: 504, retryable: true, details: { url: safeText(url, '', 2000), ...safeDetailObject(details) } });
}
...
export function renderFailed(cause, details) {
  return new YunjinError(ERROR_CODES.RENDER_FAILED, zh(28210, 26579, 22833, 36133), { status: 503, retryable: true, cause, details });
}
...
export function confirmationRequired(token, details) {
  return new YunjinError(ERROR_CODES.CONFIRMATION_REQUIRED, zh(38656, 35201, 30830, 35748, 25805, 20316), { status: 409, details: { token: safeText(token, '', 200), ...safeDetailObject(details) } });
}
...
export function normalizeError(error, fallbackCode = ERROR_CODES.INTERNAL) {
  ...
  return new YunjinError(fallbackCode, safeText(message, zh(20869, 37096, 38169, 35823)), { cause: error, expose: false });
}
...
export function publicErrorMessage(error, fallback) {
  ...
  if (normalized.code === ERROR_CODES.NETWORK_TIMEOUT) return zh(32593, 32476, 35831, 27714, 36229, 26102, 65292, 35831, 31245, 21518, 20877, 35797);
  if (normalized.code === ERROR_CODES.NETWORK_RESPONSE) return zh(22806, 37096, 26381, 21153, 36820, 22238, 24322, 24120);
  if (normalized.expose) return normalized.message;
  return zh(25805, 20316, 22833, 36133);
}

export function assertCondition(condition, errorFactory) {
  if (condition) return;
  throw typeof errorFactory === 'function' ? errorFactory() : invalidInput(String(errorFactory || zh(26465, 20214, 19981, 28385, 36275)));
}
