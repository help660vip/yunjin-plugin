...
const featuresRoot = path.join(root, 'apps', 'features');
...
const template = [
  ...
  "export const featureSlug = '__SLUG__';",
...
async function main() {
  const directories = await fs.readdir(featuresRoot, { withFileTypes: true });
  const generated = [];
  for (const manifest of featureManifests) {
    const id = String(manifest.id).padStart(2, '0');
    const directory = directories.find((entry) => entry.isDirectory() && entry.name.startsWith(id + '-'));
    if (!directory) throw new Error('missing feature directory ' + id);
    const file = path.join(featuresRoot, directory.name, 'service.js');
    const source = template
      .replaceAll('__ID__', id)
      .replaceAll('__SLUG__', manifest.slug)
      .replaceAll('__COMMAND__', manifest.command);
    await fs.writeFile(file, source + '\n', 'utf8');
    generated.push(file);
  }
  process.stdout.write('generated feature services: ' + generated.length + '\n');
}
