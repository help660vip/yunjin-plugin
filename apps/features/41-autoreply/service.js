...
export const featureId = '41';
export const featureSlug = 'autoreply';
...
export const command = String.fromCodePoint(0x81ea, 0x52a8, 0x56de, 0x590d);
...
export function healthCheck(runtime) {
  const healthInfo = health(runtime);
  return { ...healthInfo, degraded: isDegraded(runtime), operations: actions.length, policy: { rateWindowMs: operationPolicy.rateWindowMs, dailyQuota: operationPolicy.dailyQuota } };
}
...
export function renderPolicyFor(runtime) {
  const renderer = runtime?.render || runtime?.core?.render || runtime?.renderer;
  return { ...renderPolicy, renderer: renderer ? 'runtime' : 'text', degraded: !renderer };
}
